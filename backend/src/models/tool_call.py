...
class ToolCall(SQLModel, table=True):
    ...
    message_id: Optional[int] = Field(default=None, foreign_key="messages.id")  # Nullable since not all tool calls are tied to a specific message
    tool_name: str = Field(max_length=255, nullable=False)  # Name of the MCP tool called
    tool_input: Dict[str, Any] = Field(default={})  # Input parameters passed to the tool (JSON)
    tool_output: Optional[Dict[str, Any]] = Field(default=None)  # Output received from the tool (JSON)
